...
import { createMedicalRecord } from "@/server/actions/medicalRecord";
...
export function MedicalRecordForm({ patientId }: MedicalRecordFormProps) {
  ...
  const onSubmit = async (values: MedicalRecordFormValues) => {
    ...
    try {
      ...
      setSubmitError("No se pudo guardar el historial.");
    }
  };
    ...
    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
      <div className="space-y-2">
        ...
        {errors.consultationType ? (
          <p className="text-xs text-red-600">{errors.consultationType.message}</p>
        ) : null}
      </div>
      <div className="space-y-2">
      ...
      <div className="space-y-2">
        ...
        <Textarea id="chiefComplaint" rows={3} {...form.register("chiefComplaint")} />
      ...
      <div className="space-y-2">
        ...
        <Textarea id="personalHistory" rows={3} {...form.register("personalHistory")} />
      </div>
      <div className="space-y-2">
      ...
      <Button type="submit" disabled={isSubmitting}>
        {isSubmitting ? "Guardando..." : "Agregar historial"}
      </Button>
    </form>
  );
}
